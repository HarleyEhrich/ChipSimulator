...
TriStateGate::TriStateGate():
    AbstractConInterface(){
    init();
}

TriStateGate::TriStateGate(long sceneID, QGraphicsItem *parent, QObject *parentObject):
    AbstractConInterface(sceneID, parent, parentObject){
    init();
}

void TriStateGate::run(){
    runImpl();
    update();
}

QFont TriStateGate::font() const{
    return _font;
}

void TriStateGate::init(){
    if(false == __intialSta){
        IntialStaticVariable();
        IntialComponentInfo(this,
                            "001",
                            "TriStateGate",
                            "三态门，在EN端为高电平时，输出端为高阻态",
                            "Bulid In",
                            "2023-05-01",
    ...
    connect(this,&TriStateGate::tellComNickNameChange,this,&TriStateGate::updateStatus);
    updateStatus();
}

void TriStateGate::runImpl(){
    if(_input2.lock()->getDataValue(0)){
        _output.lock()->setHighResistance(false);
        _output.lock()->setDataValue(QBitArray{1,_input1.lock()->getDataValue(0)},0);
    }else{
        _output.lock()->setHighResistance(true);
    }
}

void TriStateGate::updateStatus(){
    gneratePos();
    gnerateItemPainterPathImpl();
}

void TriStateGate::gneratePos(){
    QFontMetrics fm{_font};
    _gateNamePos.setX(-fm.boundingRect(_gateName).width()/2);
    _gateNamePos.setY(-fm.boundingRect(_gateName).height()-4);
}

void TriStateGate::gnerateItemPainterPathImpl(){
...
void TriStateGate::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget){
    Q_UNUSED(option)
    Q_UNUSED(widget)
...
AbstractConInterface *TriStateGate::instanceImpl(long sceneID, QGraphicsItem *parent){
    return new TriStateGate(sceneID,parent);
}

void TriStateGate::gnerateItemPainterPath(){
    gnerateItemPainterPathImpl();
}

bool TriStateGate::loadFromXmlExtendImpl(QXmlStreamReader *root){
    Q_UNUSED(root);
    return true;
}

bool TriStateGate::saveToXmlExtendImpl(QXmlStreamWriter *root){
    Q_UNUSED(root);
    return true;
}

void TriStateGate::connectionDataChangeImpl(UniConnectionPoint *changePtr, qsizetype changedIndex, int changeLen){
    Q_UNUSED(changePtr);
...
void TriStateGate::IntialStaticVariable(){
