...
AndGate::AndGate(long sceneID, QGraphicsItem *parent, QObject *parentObject):
...
AbstractConInterface *AndGate::instanceImpl(long sceneID, QGraphicsItem *parent){
    return new AndGate(sceneID,parent);
}

void AndGate::gnerateItemPainterPath(){
    gnerateItemPainterPathImpl();
}

bool AndGate::loadFromXmlExtendImpl(QXmlStreamReader *root){
    Q_UNUSED(root);
    return true;
}

bool AndGate::saveToXmlExtendImpl(QXmlStreamWriter *root){
    Q_UNUSED(root);
    return true;
}

void AndGate::connectionDataChangeImpl(UniConnectionPoint *changePtr, qsizetype changedIndex, int changeLen){
    Q_UNUSED(changePtr);
...
void AndGate::IntialStaticVariable(){
